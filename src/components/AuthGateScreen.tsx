...
export const AuthGateScreen: React.FC<AuthGateScreenProps> = ({
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (legacyProfile) {
      try {
        ...
        if (migrationErr.code === 'auth/email-already-in-use') {
          // Attempt sign-in with migrated credentials
          try {
    ...
    // 2. Active Dev Mode bypass for demo users (Category B)
    if (isDevMode) {
    ...
    if (lastError?.code === 'auth/wrong-password' || lastError?.code === 'auth/invalid-credential') {
      ...
      setLoginError('Пользователь с таким никнеймом или email не найден. Зарегистрируйтесь.');
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setRegError('Регистрация по email/паролю отключена в Firebase. Воспользуйтесь входом через Google.');
        setIsSubmitting(false);
        return;
