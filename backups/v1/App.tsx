...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'notes' | 'news'>('notes');
  ...
  // Live RSS Feeds Refresh (with max 10 articles per source limit + LLM formatting pipeline)
  const handleRefresh = useCallback(async (overrideFeeds?: FeedSource[]) => {
    setIsRefreshing(true);
    setRefreshStatusMessage('Подключение к источникам и сканирование публикаций...');
    try {
      ...
      setRefreshStatusMessage(`Загрузка свежих публикаций из ${activeFeeds.length} каналов...`);
      const results = await Promise.allSettled(
        activeFeeds.map((f) => fetchFeedArticles(f, 10))
      );

      const rawArticles: Article[] = [];
      results.forEach((res) => {
        if (res.status === 'fulfilled' && res.value?.articles && Array.isArray(res.value.articles)) {
          rawArticles.push(...res.value.articles.slice(0, 10));
        }
      });

      setRefreshStatusMessage('Загрузка завершена. Очистка от рекламы и фильтрация дубликатов...');
      ...
      rawArticles.forEach((art) => {
        ...
        const linkKey = (art.link || '').trim().toLowerCase();

        // Deduplicate
        if (!currentKeys.has(titleKey) && 
            (!art.link || !currentKeys.has(linkKey)) &&
            !seenSet.has(titleKey) &&
            (!art.link || !seenSet.has(linkKey))) {
          
          uniqueNewArticles.push(art);
          currentKeys.add(titleKey);
          if (art.link) currentKeys.add(linkKey);
        }
      });

      if (uniqueNewArticles.length > 0) {
        setRefreshStatusMessage(`Найдено ${uniqueNewArticles.length} новых материалов. Подключение к Gemini 3.7 Flash...`);
        // Mark as seen
        const newlySeen: string[] = [];
        uniqueNewArticles.forEach((a) => {
          if (a.title) newlySeen.push(a.title.trim().toLowerCase());
          if (a.link) newlySeen.push(a.link.trim().toLowerCase());
        });
        markArticlesAsSeen(newlySeen);
        ...
        // Trigger AI formatting & Russian translation for new articles
        const unformatted = uniqueNewArticles.filter(a => !a.summaryOneLine || !a.summaryThreeLines).slice(0, 15);
        if (unformatted.length > 0) {
          ...
          setRefreshStatusMessage('Все материалы уже оптимизированы ИИ.');
        }
      } else {
        // No new unique articles found. Let's re-process the existing top articles using the current (potentially modified) custom prompt!
        setRefreshStatusMessage('Новых новостей нет. Применение вашего ИИ-промпта к текущим 15 карточкам...');
        const targetArticles = articles.slice(0, 15);
        if (targetArticles.length > 0) {
          const processed = await aiProcessArticles(targetArticles, customAiPrompt);
          if (processed && processed.length > 0) {
            setArticles((current) => {
              const pMap = new Map<string, Article>();
              processed.forEach(p => pMap.set(p.id, p));
            ...
            setRefreshStatusMessage('Карточки успешно переформатированы по новому промпту!');
            playUiSound('success');
          } else {
            setRefreshStatusMessage('Обновление завершено. Лента актуальна.');
          }
        } else {
          setRefreshStatusMessage('Обновление завершено. Лента пуста.');
        }
      }
  ...
  const handleReprocessAllArticles = useCallback(async () => {
    ...
    try {
      ...
      if (mismatchedArticles.length > 0) {
        setRefreshStatusMessage(`Переработка ${mismatchedArticles.length} карточек в ИИ-шаблон через Gemini 3.7...`);
        ...
        <div className="bg-[#0e121a]/95 backdrop-blur-md border-b border-[#ffcc00]/30 text-[#ffcc00] px-4 py-2 flex items-center justify-between font-mono text-[11px] shrink-0 z-30 shadow-md animate-in slide-in-from-top duration-200">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <span className="text-slate-200 truncate font-sans text-xs">
              {refreshStatusMessage || 'Обработка и адаптация контента через Gemini 3.7 Flash...'}
            </span>
          </div>
